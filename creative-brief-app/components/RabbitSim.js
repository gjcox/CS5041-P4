...
export default function RabbitSim() {
  ...
  function incrementCounters() {
    ...
    if (
      !checkNearDawnDusk(
        simEnvData.season.value,
        simEnvData.time.value,
        hoursAwake
      )
    ) {
  ...
  function setActivityMsgWrapper(newMsg) {
    ...
    if (newMsg != activityMsg) {
      ...
      setActivityMsg(
        rabbitFacts[Math.floor(Math.random() * rabbitFacts.length)]
      );
      setLastActivityMsgTime(Date.now());
    }
  }
  ...
  useEffect(() => {
    let nowRaining =
      simEnvData.humidity.value >= 1.5 * simEnvData.temp.value + 60;
    if (nowRaining != raining) {
      setRaining(nowRaining);
      console.log("raining: ", nowRaining);
      setObservationMsgWrapper(
        `It has ${nowRaining ? "started" : "stopped"} raining.`
      );
    }
  }, [simEnvData]);
    ...
    if (rabbitInside && rabbitActivity == Activity.sleep && !rabbitWakeful) {
      ...
      setActivityMsgWrapper(
        buildMessage(
          ...
          true,
          Activity.sleep,
          `rabbits are crepuscular and it is ${getTimeFromMinutes(
            simEnvData.time.value
          )} in ${seasons[simEnvData.season.value]}`
        )
      );
      ...
      if (!rabbitInside) {
        ...
        if (simEnvData.visitor.value) {
          ...
          setActivityMsgWrapper(
            buildMessage(true, true, Activity.shelter, badWeather)
          );
          setActivity(true, Activity.shelter);
        } else if (simEnvData.randomChoice.value <= 33) {
          setActivityMsgWrapper(
            buildMessage(false, false, Activity.feed, "I am hungry")
          );
          ...
          setActivityMsgWrapper(
            buildMessage(
              false,
              false,
              `${Activity.play} with the grey rabbit`,
          ...
          setActivityMsgWrapper(
            buildMessage(false, false, Activity.exercise, "I want to")
          );
        ...
        if (
          ...
          setActivityMsgWrapper(
            buildMessage(
              ...
              true,
              Activity.hide,
              "I heard a visitor outside"
            )
          );
          setActivity(true, Activity.hide);
        } else if (badWeather) {
          setActivityMsgWrapper(
            buildMessage(false, true, Activity.shelter, `${badWeather} outside`)
          );
          setActivity(true, Activity.shelter);
        } else if (simEnvData.randomChoice.value <= 20) {
          setActivityMsgWrapper(
            buildMessage(false, true, Activity.feed, "I am hungry")
          );
          ...
          setActivityMsgWrapper(
            buildMessage(
              false,
              true,
              `${Activity.play} with the white rabbit`,
              "I am feeling playful"
            )
          );
          ...
          setActivityMsgWrapper(
            buildMessage(true, false, Activity.feed, "I am hungry")
          );
          setActivity(false, Activity.feed);
          ...
          setActivityMsgWrapper(
            buildMessage(true, false, Activity.exercise, "I want to")
          );
          setActivity(false, Activity.exercise);
        }
      }
